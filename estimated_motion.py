...
def estimate_motion(image1_path, image2_path):
    ...
    return img1_warped, img_matches, H

def main():
    parser = argparse.ArgumentParser(description="Vanishing Point Detection and Motion Estimation")
    parser.add_argument('-i1', '--image1-path', required=True, help='Path to the first input image')
    parser.add_argument('-i2', '--image2-path', required=True, help='Path to the second input image')
    ...
    parser.add_argument('-f', '--focal-length', default=1500, type=float, help='Focal length of the camera (in pixels)')
    ...
    parser.add_argument('-dp', '--debug-path', default=None, help='Path for writing the debug image')
    parser.add_argument('-s', '--seed', default=None, type=int, help='Specify random seed for reproducible results')
    args = parser.parse_args()

    try:
        vpd = VPDetection(length_thresh=args.length_thresh,
                          principal_point=args.principal_point,
                          focal_length=args.focal_length,
                          seed=args.seed)

        # Detect vanishing points for the first image
        vps1 = vpd.find_vps(args.image1_path)
        print("\nVanishing Points for Image 1 in 3D space:")
        for i, vp in enumerate(vps1):
            print(f"Vanishing Point {i + 1}: {vp}")
        print("\nVanishing Points for Image 1 in image coordinates:")
        for i, vp in enumerate(vpd.vps_2D):
            print(f"Vanishing Point {i + 1}: {vp}")

        if args.debug or args.debug_show:
            vpd.create_debug_VP_image(args.debug_show, args.debug_path)

        # Detect vanishing points for the second image
        vps2 = vpd.find_vps(args.image2_path)
        print("\nVanishing Points for Image 2 in 3D space:")
        for i, vp in enumerate(vps2):
            print(f"Vanishing Point {i + 1}: {vp}")
        print("\nVanishing Points for Image 2 in image coordinates:")
        for i, vp in enumerate(vpd.vps_2D):
            print(f"Vanishing Point {i + 1}: {vp}")
        ...
        # Estimate motion between the two images
        img1_warped, img_matches, H = estimate_motion(args.image1_path, args.image2_path)
        cv2.imwrite("warped_image1.jpg", img1_warped)
        cv2.imwrite("matches.jpg", img_matches)

        print("\nEstimated Homography Matrix:")
        print(H)
        print(f"\nSaved warped image as 'warped_image1.jpg' and matches as 'matches.jpg'")
