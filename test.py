...
# Generate initial and subsequent poses
initial_pose = np.eye(4)
initial_pose[:3, :3] = heading_to_rotation_matrix(initial_heading)
initial_pose[:3, 3] = [latitude, longitude, 0]

# Calculate poses for filtered points
poses = [initial_pose]
for i in filtered_indices[1:]:
    new_lat, new_lon = lat_lon_points[i]
    translation = [new_lat - latitude, new_lon - longitude, 0]
    new_pose = np.eye(4)
    new_pose[:3, :3] = heading_to_rotation_matrix(initial_heading)
...
# Save poses to a file (optional)
poses_output_file = os.path.join(output_dir, 'poses.npy')
np.save(poses_output_file, poses)
print(f"Poses saved to {poses_output_file}")
